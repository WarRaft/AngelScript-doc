...
Логические
=
Представлены классическим [bool](https://www.angelcode.com/angelscript/sdk/docs/manual/doc_datatypes_primitives.html#bool) со значениями **true** или **false**.
...
Если не указано иное, то первая константа принимает значение __0__. Остальные принимают значение равное увеличенному на единицу значению предыдущей:
((код cpp
enum MyEnum {
A, // 0
B, // 1
...
Все типы, объявленные в [common.j](https://github.com/UnryzeC/UjAPI/blob/main/uJAPIFiles/common.j) внутри языка являются **uint32** и уничтожается именно это число, а не внутриигровой объект.
...
=== Присваивание ===
Строки, как и примитивы, присваиваются по значению с помощью [оператора](https://xgm.guru/p/ij/angelscript-operator) присваивания.
...
=== Конкатенация ===
В первом приближении [конкатенация](https://w.wiki/9eWw) это просто склеивание нескольких строк в одну с помощью [операторов](https://xgm.guru/p/ij/angelscript-operator) сложения.
...
=== Равенство ===
Применяет [оператор](https://xgm.guru/p/ij/angelscript-operator) равенства к каждому байту строки. //Строки разной длины не равны, что логично.
...
**str** - строка
**base** - основание [целого числа](https://xgm.guru/p/ij/angelscript-data-types#tselye-chisla), принимает два значения: **10** или **16**
**byteCount** - [выходное значение](https://xgm.guru/p/ij/angelscript-function#out) в которое устанавливается количество байт, которые были учтены как значение
